...
void Interfaz::imprimir_arbol_r_recursivo(Nodo* nodo, int &espacio, int color){
    ...
    else
    {
        for (auto i : nodo->entradas)
        {
            int dx = i->intervalos[0].mayor - i->intervalos[0].menor;
            int dy = i->intervalos[1].mayor - i->intervalos[1].menor;
            ...
            rectangulo.setPosition(i->intervalos[0].menor, window.getSize().y - i->intervalos[1].mayor);
